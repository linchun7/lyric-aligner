...
def parse_song_list(path: Path) -> list[SongEntry]:
    ...
    if any(right.nominal_start_ms < left.nominal_start_ms for left, right in zip(entries, entries[1:])):
...
def _candidate_score(entry: SongEntry, path: Path) -> float:
    ...
    combined_score = difflib.SequenceMatcher(None, artist_title, stem, autojunk=False).ratio()
...
def rank_candidates(entry: SongEntry, candidates: Iterable[Path]) -> list[CandidateScore]:
    return sorted(
    ...
    for entry in entries:
        ...
        if asset is None:
            lyric_path, lyric_diag = choose_candidate(
                entry, lrc_files, label="LRC", min_score=min_score, min_margin=min_margin
            )
            source_path, source_diag = choose_candidate(
                entry, audio_files, label="source audio", min_score=min_score, min_margin=min_margin
            )
        ...
        occurrences.append(
            TrackOccurrence(
                ...
                middle_cut=str(middle_cut_by_occurrence.get(entry.ordinal, "false")),
