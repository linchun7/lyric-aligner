"""A-bounded text-only recovery for Smart v1.2.5.

This tier runs only after the previous Smart policy has finished text recovery
and timing validation.  It may resolve a consecutive mapped-review region only
when local canonical boundaries are already resolved and a farther pair of
same-source A-grade timing anchors brackets the region.  Those A anchors must
come from an already-ready timing model and remain within the established
750 ms residual envelope.

A-bounded recovery never recomputes or mutates timing.  Recovered text decisions
are capped below B timing authority and preserve cue count, numbering, start/end
boundaries, and existing editor segmentation.
"""
...
def _candidate_for_block(
    ...
    if not block_cues or len(block_cues) != len(block_decisions) or not gap:
        return None
    if any(
        item.action != "review" or _mapped_span(item) is None
...
def recover_mapped_reviews_from_a_bounded(
    cues: Sequence[SubtitleCue],
    canonical: Sequence[TimedCanonicalOccurrence],
    decisions: Sequence[MatchDecision],
    timing: Sequence[TimingDecision],
) -> tuple[dict[int, str], list[MatchDecision], ABoundedRecoverySummary]:
    ...
    for block_start, block_end in _review_blocks(cues, decisions_by_cue):
        ...
        if (
            ...
            or any(
                row.source_ordinal != source_ordinal for row in left_boundary_rows
            )
            or any(
                row.source_ordinal != source_ordinal for row in right_boundary_rows
            )
        ):
            continue
        ...
        for item in typed_decisions:
            ...
            if not rows or any(
                row.source_ordinal != source_ordinal for row in rows
            ):
                mapped_ok = False
                break
