...
from lyric_aligner.timeline.partial_repair import CueTrust, PartialTimelineRepairError, TimingCandidate
...
def _load_json(path: Path) -> dict[str, Any]:
    try:
        ...
        raise PartialTimelineRepairError(f"cannot read lineage JSON: {path.name}") from exc
    if not isinstance(payload, dict):
        raise PartialTimelineRepairError(f"lineage JSON must be an object: {path.name}")
    return payload
...
def _validate_pair(
    *,
    ...
    if payload.get("task_fingerprint_sha256") != fingerprint:
        raise PartialTimelineRepairError(f"{stage} payload belongs to another task")
    if payload.get("algorithm_version") != algorithm_version:
        raise PartialTimelineRepairError(f"{stage} payload algorithm version mismatch")
...
def _required_path(row: dict[str, Any], key: str) -> Path:
    ...
    if not value:
        raise PartialTimelineRepairError(f"effective occurrence is missing {key}")
    path = Path(value)
    if not path.is_file():
        raise PartialTimelineRepairError(f"effective occurrence lineage file is missing: {path.name}")
    return path
...
def _validate_continuous_mapping(mapping: object) -> str:
    if not isinstance(mapping, dict):
        raise PartialTimelineRepairError("effective continuous TimeWarp has no mapping")
    mode = str(mapping.get("mode") or "").strip()
    if mode not in _CONTINUOUS_MODES:
        raise PartialTimelineRepairError(
            f"unsupported effective continuous mapping mode: {mode or '<missing>'}"
        )
    ...
    if not isinstance(breakpoints, list) or not isinstance(deltas, list):
        raise PartialTimelineRepairError("continuous mapping hinge arrays are invalid")
    if len(breakpoints) != len(deltas):
        raise PartialTimelineRepairError("continuous mapping hinge arrays disagree")
    if mode == "AFFINE" and (breakpoints or deltas):
        raise PartialTimelineRepairError("AFFINE mapping unexpectedly contains rate breakpoints")
    if mode == "PIECEWISE_RATE" and not breakpoints:
        raise PartialTimelineRepairError("PIECEWISE_RATE mapping has no rate breakpoint")
    return mode
...
def _derive_continuous_context(
    *,
    ...
    coarse_artifact_path = _required_path(occurrence, "coarse_artifact_path")
    ...
    if str(coarse.get("occurrence_id") or "") != occurrence_id:
        raise PartialTimelineRepairError("coarse/effective-run occurrence identity mismatch")
    ...
    if mapping_source == "coarse":
        ...
        if not isinstance(timewarp, dict):
            raise PartialTimelineRepairError("coarse alignment has no TimeWarp")
    ...
    if mapping_source != "fine":
        raise PartialTimelineRepairError(
            f"unsupported continuous mapping_source: {mapping_source or '<missing>'}"
        )
    ...
    fine_artifact_path = _required_path(occurrence, "fine_artifact_path")
    ...
    if str(fine.get("occurrence_id") or "") != occurrence_id:
        raise PartialTimelineRepairError("fine/effective-run occurrence identity mismatch")
    if coarse_id not in {
        str(value) for value in fine_artifact.get("upstream_artifact_ids", [])
    }:
        raise PartialTimelineRepairError("Fine artifact is not derived from effective coarse")
    fine_result = fine.get("result")
    if not isinstance(fine_result, dict) or fine_result.get("applied") is not True:
        raise PartialTimelineRepairError("effective Fine payload is not applied")
    timewarp = fine_result.get("timewarp")
    if not isinstance(timewarp, dict) or bool(timewarp.get("blocked")):
        raise PartialTimelineRepairError("effective Fine TimeWarp is missing or blocked")
    mode = _validate_continuous_mapping(timewarp.get("mapping"))
    return OccurrenceMappingContext(
        occurrence_id=occurrence_id,
        status="ready",
...
def _derive_cut_context(
    *,
    ...
    artifact_path = _required_path(occurrence, "cut_mapping_artifact_path")
    ...
    if str(payload.get("occurrence_id") or "") != occurrence_id:
        raise PartialTimelineRepairError("cut mapping/effective-run occurrence identity mismatch")
    result = payload.get("result")
    if not isinstance(result, dict) or result.get("kind") != "CUT_AWARE":
        raise PartialTimelineRepairError("materialized cut mapping is not CUT_AWARE")
    cuts = result.get("cuts")
    segments = result.get("segments")
    if not isinstance(cuts, list) or not cuts:
        raise PartialTimelineRepairError("CUT_AWARE mapping has no materialized cut")
    if not isinstance(segments, list) or len(segments) < 2:
        raise PartialTimelineRepairError("CUT_AWARE mapping has insufficient retained segments")
    cut_count = len(cuts)
    try:
        summary_cut_count = int(occurrence.get("cut_count", -1))
        artifact_cut_count = int(artifact.get("evidence", {}).get("cut_count", -1))
    except (TypeError, ValueError) as exc:
        raise PartialTimelineRepairError("materialized cut count is invalid") from exc
    if summary_cut_count != cut_count or artifact_cut_count != cut_count:
        raise PartialTimelineRepairError("CUT_AWARE cut count disagrees across lineage")
    if str(artifact.get("evidence", {}).get("occurrence_id") or "") != occurrence_id:
        raise PartialTimelineRepairError("cut artifact evidence occurrence identity mismatch")
    confirmed_candidate_ids = artifact.get("normalized_config", {}).get(
        "confirmed_candidate_ids"
    )
    if not isinstance(confirmed_candidate_ids, list) or len(confirmed_candidate_ids) != cut_count:
        raise PartialTimelineRepairError("cut artifact confirmed candidate identity is incomplete")
    if any(not str(value).strip() for value in confirmed_candidate_ids):
        raise PartialTimelineRepairError("cut artifact contains empty confirmed candidate identity")

    cut_meta = run.get("cut_rebuild")
    if not isinstance(cut_meta, dict):
        raise PartialTimelineRepairError("effective cut run is missing cut_rebuild metadata")
    review_id = str(cut_meta.get("source_review_artifact_id") or "").strip()
    if not review_id:
        raise PartialTimelineRepairError("cut_rebuild metadata has no source review artifact")
    if artifact.get("normalized_config", {}).get("source_review_artifact_id") != review_id:
        raise PartialTimelineRepairError("cut mapping belongs to another review materialization")
    if review_id not in {
        str(value) for value in artifact.get("upstream_artifact_ids", [])
    }:
        raise PartialTimelineRepairError("cut mapping artifact does not bind its review artifact")
    if review_id not in effective_upstreams:
        raise PartialTimelineRepairError("source review artifact is not upstream of effective run")
...
def derive_effective_run_mapping_context(
    *,
    ...
    """Derive mapping kind/cut identity without caller-supplied mapping labels."""

    run = _load_json(run_path)
    artifact = _load_json(run_artifact_path)
    fingerprint = str(run.get("task_fingerprint_sha256") or "").strip()
    algorithm_version = str(run.get("algorithm_version") or "").strip()
    if not fingerprint or not algorithm_version:
        raise PartialTimelineRepairError("effective run is missing task/algorithm identity")
    stage = str(artifact.get("stage") or "").strip()
    role = _RUN_ROLES.get(stage)
    if role is None:
        raise PartialTimelineRepairError(f"unsupported effective run stage: {stage or '<missing>'}")
    ...
    if validated_run is not run and validated_run != run:
        raise PartialTimelineRepairError("effective run changed during lineage validation")
    if validated_run.get("legacy_fallback_used") is not False:
        raise PartialTimelineRepairError("partial repair refuses a legacy-fallback run")
    run_artifact_id = str(validated_artifact.get("artifact_id") or "").strip()
    effective_upstreams = {
        str(value)
    ...
    if not isinstance(rows, list) or not rows:
        raise PartialTimelineRepairError("effective run has no occurrences")
    seen: set[str] = set()
    contexts: list[OccurrenceMappingContext] = []
    for occurrence in rows:
        if not isinstance(occurrence, dict):
            raise PartialTimelineRepairError("effective run occurrence must be an object")
        occurrence_id = str(occurrence.get("occurrence_id") or "").strip()
        if not occurrence_id or occurrence_id in seen:
            raise PartialTimelineRepairError("effective run occurrence identity is missing/duplicated")
        ...
        if mapping_source == "cut_aware_rebuild":
            ...
            raise PartialTimelineRepairError(
                f"effective run has unsupported mapping_source {mapping_source!r}"
            )
...
def bridge_effective_run_to_partial_repair(
    *,
    ...
    if fusion.get("source_run_artifact_id") != context.run_artifact_id:
        raise PartialTimelineRepairError("P9 fusion is not bound to the effective run artifact")
