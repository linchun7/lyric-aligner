...
def find_strong_profanity(text: str) -> list[dict[str, object]]:
    """Return deterministic strong-profanity spans as review hints only."""

    return [
        {"start": match.start(), "end": match.end(), "text": match.group(0)}
        for match in _STRONG_PROFANITY_RE.finditer(text)
    ]
