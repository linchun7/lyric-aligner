...
def _validate_final_render_binding(
    ...
    *,
    ...
    report: Path,
    qa_json: Path,
    require_semantic_sensitive_review: bool = False,
) -> str:
    ...
    if require_semantic_sensitive_review:
        layers = {
        ...
        for label, layer in layers.items():
            if layer.get("semantic_sensitive_review_applied") is not True:
                raise ValueError(
                    f"v4 release blocked: {label} lacks completed semantic-sensitive review"
                )
            reviewer = str(layer.get("semantic_sensitive_reviewer_model") or "").strip()
            pack_sha = str(layer.get("semantic_sensitive_pack_sha256") or "").strip()
            review_sha = str(layer.get("semantic_sensitive_review_sha256") or "").strip()
            if not reviewer or not pack_sha or not review_sha:
                raise ValueError(
                    f"v4 release blocked: {label} lacks semantic-sensitive model/hash lineage"
                )
        if str(config.get("display_text_mask_profile") or "") != "none":
            raise ValueError(
                "v4 release blocked: semantic-sensitive final render must use display mask_profile=none"
            )
        for label, layer in (("evidence", evidence), ("qa", qa)):
            if layer.get("semantic_sensitive_full_scan_completed") is not True:
                raise ValueError(
                    f"v4 release blocked: {label} lacks full semantic-sensitive SRT scan"
                )
            if layer.get("semantic_sensitive_unresolved_review_count") != 0:
                raise ValueError(
                    f"v4 release blocked: {label} has unresolved semantic-sensitive review"
                )
            scanned = layer.get("semantic_sensitive_scanned_cue_count")
            cue_count = layer.get("cue_count")
            if type(scanned) is not int or scanned != cue_count:
                raise ValueError(
                    f"v4 release blocked: {label} semantic-sensitive scan count "
                    "does not equal final cue_count"
                )

        config_scanned = config.get("semantic_sensitive_scanned_cue_count")
        if type(config_scanned) is not int or config_scanned != qa.get("cue_count"):
            raise ValueError(
                "v4 release blocked: normalized_config semantic-sensitive scan count "
                "does not equal final cue_count"
            )
        ...
        for field in identity_fields:
            values = {str(layer.get(field) or "") for layer in layers.values()}
            if len(values) != 1:
                raise ValueError(
                    f"v4 release blocked: semantic-sensitive lineage mismatch for {field}"
                )
        for field in (
            "semantic_sensitive_masked_cue_count",
            "semantic_sensitive_masked_term_count",
        ):
            if evidence.get(field) != qa.get(field):
                raise ValueError(
                    f"v4 release blocked: semantic-sensitive count mismatch for {field}"
                )
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Historical replay only; new v4 releases require model semantic-sensitive finalization",
    )
    ...
    try:
        ...
        if args.algorithm_version.startswith("4."):
            ...
            final_render_artifact_id = _validate_final_render_binding(
                ...
                report=args.report,
                qa_json=args.qa_json,
                require_semantic_sensitive_review=(
                    not args.legacy_allow_no_semantic_sensitive_review
                ),
            )
            semantic_requested = (
