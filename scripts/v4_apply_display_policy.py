...
from lyric_aligner.review.semantic_sensitive import (
    materialize_semantic_masks,
    validate_semantic_sensitive_review,
)
...
def main() -> int:
    ...
    parser.add_argument("--semantic-sensitive-pack", type=Path)
    parser.add_argument("--semantic-sensitive-review", type=Path)
    parser.add_argument(
        "--legacy-allow-no-semantic-sensitive-review",
        action="store_true",
        help="Historical replay only; new production renders must provide model semantic-sensitive review",
    )
    ...
    try:
        ...
        if (args.semantic_sensitive_pack is None) != (args.semantic_sensitive_review is None):
            raise ValueError(
                "--semantic-sensitive-pack and --semantic-sensitive-review must be provided together"
            )
        if args.semantic_sensitive_pack is None:
            if not args.legacy_allow_no_semantic_sensitive_review:
                raise ValueError(
                    "new production display renders require --semantic-sensitive-pack "
                    "and --semantic-sensitive-review; legacy replay must be explicit"
                )
        elif args.legacy_allow_no_semantic_sensitive_review:
            raise ValueError(
                "--legacy-allow-no-semantic-sensitive-review cannot be combined with semantic review inputs"
            )
        ...
        if args.semantic_sensitive_pack is not None:
            protected_inputs["semantic_sensitive_pack"] = args.semantic_sensitive_pack
            protected_inputs["semantic_sensitive_review"] = args.semantic_sensitive_review
        ...
        semantic_sensitive_applied = args.semantic_sensitive_pack is not None
        semantic_sensitive_decisions: dict[int, dict] = {}
        semantic_sensitive_reviewer_model = ""
        semantic_sensitive_scanned_cue_count = 0
        semantic_sensitive_pack_sha256 = ""
        semantic_sensitive_review_sha256 = ""
        if semantic_sensitive_applied:
            if policy.mask_profile != "none":
                raise ValueError(
                    "semantic-sensitive model review requires display mask_profile=none; "
                    "mechanical profanity masking must not run before/alongside model decisions"
                )
            assert args.semantic_sensitive_pack is not None
            assert args.semantic_sensitive_review is not None
            sensitive = validate_semantic_sensitive_review(
                args.semantic_sensitive_pack,
                args.semantic_sensitive_review,
                expected_source_srt=args.source_srt,
            )
            semantic_sensitive_decisions = sensitive["decisions"]
            semantic_sensitive_reviewer_model = str(
                sensitive["review"]["reviewer_model"]
            )
            semantic_sensitive_scanned_cue_count = int(
                sensitive["review"]["scanned_cue_count"]
            )
            semantic_sensitive_pack_sha256 = sha256_file(args.semantic_sensitive_pack)
            semantic_sensitive_review_sha256 = sha256_file(args.semantic_sensitive_review)
        ...
        semantic_sensitive_masked_cue_count = 0
        semantic_sensitive_masked_term_count = 0
        ...
        for position, (cue, source_row) in enumerate(zip(source_cues, source_rows), start=1):
            ...
            display_reasons = list(result.reasons)
            semantic_decision = semantic_sensitive_decisions.get(cue.number)
            semantic_mask_count_for_cue = 0
            if semantic_decision and semantic_decision["action"] == "mask":
                if result.override_applied:
                    raise ValueError(
                        f"cue {cue.number} cannot combine semantic-sensitive MASK "
                        "with a presentation override"
                    )
                display_text, semantic_masks = materialize_semantic_masks(
                    display_text,
                    semantic_decision["mask_terms"],
                )
                semantic_mask_count_for_cue = len(semantic_masks)
                semantic_sensitive_masked_cue_count += 1
                semantic_sensitive_masked_term_count += semantic_mask_count_for_cue
                display_reasons.append("semantic_sensitive_mask")
            ...
            row["display_text_changed"] = "true" if display_text != cue.text else "false"
            row["display_change_reasons"] = ";".join(display_reasons)
            row["display_policy_id"] = policy.policy_id
            row["display_reviewer_model"] = policy.reviewer_model
            row["semantic_sensitive_action"] = (
                str(semantic_decision["action"]) if semantic_decision else ""
            )
            row["semantic_sensitive_reviewer_model"] = (
                semantic_sensitive_reviewer_model if semantic_sensitive_applied else ""
            )
            row["semantic_sensitive_mask_count"] = str(semantic_mask_count_for_cue)
            row["text_sha256"] = text_sha256(display_text)
        ...
        for name in (
            ...
            "semantic_sensitive_action",
            "semantic_sensitive_reviewer_model",
            "semantic_sensitive_mask_count",
        ):
        ...
        production_qa = {
            **source_qa,
            ...
            "semantic_sensitive_review_applied": semantic_sensitive_applied,
            "semantic_sensitive_reviewer_model": semantic_sensitive_reviewer_model,
            "semantic_sensitive_full_scan_completed": semantic_sensitive_applied,
            "semantic_sensitive_scanned_cue_count": semantic_sensitive_scanned_cue_count,
            "semantic_sensitive_pack_sha256": semantic_sensitive_pack_sha256,
            "semantic_sensitive_review_sha256": semantic_sensitive_review_sha256,
            "semantic_sensitive_masked_cue_count": semantic_sensitive_masked_cue_count,
            "semantic_sensitive_masked_term_count": semantic_sensitive_masked_term_count,
            "semantic_sensitive_unresolved_review_count": 0,
