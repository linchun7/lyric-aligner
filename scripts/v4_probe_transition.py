#!/usr/bin/env python3
"""Safely enter the unchanged V4 transition-probe implementation."""
...
_IMPL_PATH = Path(__file__).with_name("_v4_probe_transition_impl.txt")


def _execute_implementation() -> None:
    module_name = globals()["__name__"]
    globals()["__name__"] = "_lyric_aligner_v4_probe_transition_impl"
    try:
...
def main() -> int:
    try:
        validate_primary_stage_writer_from_argv(sys.argv[1:], stage="probe_transition")
    except (OSError, ValueError) as exc:
        print(f"v4_probe_transition.py: error: {exc}", file=sys.stderr)
        return 2
