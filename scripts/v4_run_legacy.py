#!/usr/bin/env python3
"""Safely enter the unchanged v4 legacy production core."""
...
_IMPL_PATH = Path(__file__).with_name("_v4_run_legacy_impl.txt")


def _execute_implementation() -> None:
    module_name = globals()["__name__"]
    globals()["__name__"] = "_lyric_aligner_v4_run_legacy_impl"
    try:
...
def main() -> int:
    try:
        validate_run_output_tree_from_argv(sys.argv[1:])
    except (OSError, ValueError) as exc:
        print(f"v4_run_legacy.py: error: {exc}", file=sys.stderr)
        return 2
