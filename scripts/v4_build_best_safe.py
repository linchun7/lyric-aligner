...
from lyric_aligner.review.semantic_sensitive import build_semantic_sensitive_pack
...
def main() -> int:
    ...
    parser.add_argument("--mask-profile", choices=("strong_profanity_v1", "none"), default="none")
    ...
    sensitive_pack_path = out_dir / "SENSITIVE_PACK.json"
    try:
        if sensitive_pack_path.exists():
            raise BestSafeError(
                f"semantic-sensitive review pack already exists: {sensitive_pack_path}"
            )
        ...
        sensitive_pack = build_semantic_sensitive_pack(
            out_dir / "FINAL.srt",
            song_list=args.song_list.resolve(),
        )
        sensitive_pack_path.parent.mkdir(parents=True, exist_ok=True)
        with sensitive_pack_path.open("x", encoding="utf-8") as handle:
            json.dump(sensitive_pack, handle, ensure_ascii=False, indent=2)
            handle.write("\n")
        ...
        json.dumps(
            {
                ...
                "pre_release_publish_ready": qa["pre_release_publish_ready"],
                "formal_safe_final_ready": qa["formal_safe_final_ready"],
                "semantic_sensitive_pack": str(sensitive_pack_path),
                "semantic_sensitive_full_scan_required": True,
            },
            ensure_ascii=False,
        )
    )
