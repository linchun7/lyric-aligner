#!/usr/bin/env python3
"""Safely enter the unchanged V4 coarse-alignment implementation."""
...
_IMPL_PATH = Path(__file__).with_name("_v4_coarse_align_impl.txt")


def _execute_implementation() -> None:
    module_name = globals()["__name__"]
    globals()["__name__"] = "_lyric_aligner_v4_coarse_align_impl"
    try:
...
def main() -> int:
    try:
        validate_primary_stage_writer_from_argv(sys.argv[1:], stage="coarse_align")
    except (OSError, ValueError) as exc:
        print(f"v4_coarse_align.py: error: {exc}", file=sys.stderr)
        return 2
