...
def _load_bounded_mix(
    ...
    *,
    ...
    if len(audio) + 1 < required_samples:
        raise ValueError(
            "bounded mix decode ended before requested occurrence interval"
        )
...
def main() -> int:
    ...
    try:
        ...
        assets = json.loads(
            args.track_assets.read_text(encoding="utf-8-sig")
        )
        ...
        if binding is None:
            raise ValueError(
                "occurrence_id not found in track assets: "
                f"{args.occurrence_id}"
            )
        ...
        step_seconds = (
            defaults.step_seconds
            if args.step_seconds is None
            else args.step_seconds
        )
        ...
        mix_start = (
            default_start if args.mix_start is None else args.mix_start
        )
        ...
        feature_cache_dir = (
            ...
            or _default_feature_cache_dir(args.out)
        )
