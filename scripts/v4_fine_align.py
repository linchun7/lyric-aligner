...
from lyric_aligner.audio.fine_alignment import (
    refine_coarse_mapping,
    should_run_fine_alignment,
)
...
def _validate_stage(
    ...
    *,
    ...
    issues.extend(
        validate_artifact_output(artifact, role=role, path=path)
    )
    if issues:
        raise ValueError(
            f"invalid {role} artifact: " + "; ".join(issues)
        )
...
def _mix_window_bounds(coarse: dict) -> tuple[float, float]:
    ...
    if not isinstance(windows, list) or not windows:
        raise ValueError(
            "fine alignment requires coarse retrieval windows"
        )
    try:
        ...
        raise ValueError(
            "coarse retrieval windows have invalid mix bounds"
        ) from exc
    if start < 0 or end <= start:
        raise ValueError(
            "coarse retrieval windows have invalid mix interval"
        )
    return start, end
...
def _load_bounded_mix(
    ...
    *,
    ...
    decode_end = min(
        full_mix_duration,
        mix_end + MIX_DECODE_PADDING_SECONDS,
    )
    ...
    if len(audio) + 1 < required_samples:
        raise ValueError(
            "bounded mix decode ended before requested fine interval"
        )
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=float,
        help=(
            "Experimental override; release is blocked until moved into "
            "the asset profile."
        ),
    )
    parser.add_argument(
    ...
    try:
        ...
        assert_manifest_paths(
            args.task_manifest,
            task,
            {"audio": args.mix_audio},
        )
        fingerprint = str(task["task_fingerprint_sha256"])
        verified_mix_sha256 = str(task["inputs"]["audio"]["sha256"])
        ...
        hop_length = (
            ...
            else args.hop_length
        )
        source_radius_seconds = (
            defaults.source_radius_seconds
            if args.source_radius_seconds is None
            else args.source_radius_seconds
        )
        slope_radius = (
            defaults.slope_radius
            if args.slope_radius is None
            else args.slope_radius
        )
        slope_step = (
            defaults.slope_step
            if args.slope_step is None
            else args.slope_step
        )
        candidate_step_seconds = (
            defaults.candidate_step_seconds
            if args.candidate_step_seconds is None
            else args.candidate_step_seconds
        )
        min_score = (
            defaults.min_score
        ...
        if (
            str(coarse.get("upstream_asset_artifact_id"))
            != context.asset_artifact.artifact_id
        ):
            raise ValueError(
                "coarse alignment came from a different asset artifact"
            )
        if (
            str(coarse.get("calibration_profile_id"))
            != context.calibration_profile_id
        ):
            raise ValueError(
                "coarse alignment came from a different calibration profile"
            )
        ...
        if str(coarse.get("track_id")) != binding.track_id:
            raise ValueError(
                "coarse track_id differs from resolved TrackAsset"
            )
        if (
            str(coarse.get("canonical_selection_sha256"))
            != binding.canonical_selection_sha256
        ):
            raise ValueError(
                "coarse canonical selection differs from TrackAsset"
            )
        ...
        needs_fine = should_run_fine_alignment(
            coarse,
            force=args.force,
        )
        mix_duration = float(
            librosa.get_duration(path=str(args.mix_audio))
        )
        if needs_fine:
            mix_start, mix_end = _mix_window_bounds(coarse)
            if mix_end > mix_duration + 1e-6:
                raise ValueError(
                    "coarse retrieval windows exceed mix duration"
                )
        ...
        fine = refine_coarse_mapping(
            ...
            min_excess_source_jump=(
                timewarp_defaults.min_excess_source_jump
            ),
            min_piecewise_improvement=(
                timewarp_defaults.min_piecewise_improvement
            ),
            minimum_feature_families=(
                timewarp_defaults.minimum_feature_families
            ),
        ...
        payload = {
            ...
            "mix_audio_sha256": verified_mix_sha256,
            "source_audio_sha256": binding.source_audio_sha256,
            "upstream_asset_artifact_id": (
                context.asset_artifact.artifact_id
            ),
        ...
        artifact = build_artifact_manifest(
            ...
            evidence={
                ...
                "status": fine["status"],
                "blocked": bool(
                    fine.get("timewarp", {}).get("blocked", False)
                ),
            },
        )
