#!/usr/bin/env python3
"""Run the locked human-gold calibration suite for production boundary backends.

This is a calibration/evaluation transaction only; it never mutates subtitles.
The suite refuses to start until both audit CSVs form a valid 90-point human-gold
artifact.  It then evaluates the two versioned Mandarin backend profiles across the
three independent boundary scopes (start/end/internal).  All six prediction artifacts
preserve missing hard cases as null.  A completed output directory is materialized only
after every scope produced a valid prediction + calibration artifact; statistical gate
failure is a completed diagnostic result with authority_ready=false, not a runtime
failure.  Runtime/provenance failures are retained in a sibling .failed directory and
never masquerade as a completed suite.
"""
...
from scripts.v4_ingest_human_boundary_gold import (
    build_human_boundary_gold_artifact,
    sha256_json,
    verify_lock,
)


SUITE_SCHEMA_VERSION = "human-boundary-calibration-suite-1.0"
DEFAULT_PROFILE_IDS = ("sofa_mandarin_v1", "hubertfa_mandarin_v1")
BOUNDARY_KINDS = ("start", "end", "internal")


class HumanBoundaryCalibrationSuiteError(ValueError):
    pass
...
def _load_lock(lock_path: Path) -> dict[str, Any]:
    try:
        ...
        raise HumanBoundaryCalibrationSuiteError("selection lock is unreadable JSON") from exc
    if not isinstance(payload, dict):
        raise HumanBoundaryCalibrationSuiteError("selection lock must be an object")
    return payload


def _scope_expected_scope(calibration: Mapping[str, Any]) -> dict[str, Any]:
    scope = calibration.get("scope")
    if not isinstance(scope, Mapping):
        raise HumanBoundaryCalibrationSuiteError("calibration scope is missing")
    return dict(scope)
...
def _summary_row(
    *,
    ...
    authoritative = production_calibration_artifact_is_authoritative(
        ...
        expected_scope=_scope_expected_scope(calibration),
    )
    return {
...
def run_calibration_suite(
    *,
    ...
    if not lock_path.is_file():
        raise HumanBoundaryCalibrationSuiteError("human-gold pack has no selection.lock.json")
    if destination.exists():
        raise FileExistsError("calibration suite output must be a new path")
    staging = destination.with_name(destination.name + ".staging")
    failed = destination.with_name(destination.name + ".failed")
    if staging.exists() or failed.exists():
        raise FileExistsError("stale calibration staging/failed evidence exists; preserve or resolve it explicitly")
    ...
    if any(not value for value in unique_profile_ids) or len(set(unique_profile_ids)) != len(unique_profile_ids):
        raise HumanBoundaryCalibrationSuiteError("backend profile IDs must be unique/non-empty")
    if len(unique_profile_ids) < 2:
        raise HumanBoundaryCalibrationSuiteError("production suite requires at least two backend profiles")

    # Expected preflight failures (for example blank human audit fields) happen
    # before staging exists and, critically, before any model command is resolved.
    human_gold = build_human_boundary_gold_artifact(lock_path)
    ...
    for profile_id in unique_profile_ids:
        profile = profile_getter(profile_id)
        resolved = profile.resolve(repository, language=language)
        if str(resolved.get("profile_id") or "") != profile_id:
            raise HumanBoundaryCalibrationSuiteError("resolved backend profile ID mismatch")
        resolved_profiles.append(resolved)
    if len({row["backend_id"] for row in resolved_profiles}) < 2:
        raise HumanBoundaryCalibrationSuiteError("production suite profiles do not identify two distinct backends")
    if len({row["correlation_group"] for row in resolved_profiles}) < 2:
        raise HumanBoundaryCalibrationSuiteError("production suite profiles do not identify two independent correlation groups")
    if len({row["family"] for row in resolved_profiles}) < 2:
        raise HumanBoundaryCalibrationSuiteError("production suite profiles do not occupy two evidence families")

    staging.mkdir(parents=True, exist_ok=False)
    try:
        _atomic_json(staging / "human_gold.json", human_gold)
        ...
        if len({row["backend_id"] for row in rows}) < 2 or len({row["correlation_group"] for row in rows}) < 2:
            raise HumanBoundaryCalibrationSuiteError("resolved suite does not contain two independent backend identities")
        if len(rows) != len(unique_profile_ids) * len(BOUNDARY_KINDS):
            raise HumanBoundaryCalibrationSuiteError("calibration suite scope count is incomplete")
        ...
        try:
            ...
            # Preserve the original failure and any staging evidence. Never delete
            # diagnostic material in a secondary cleanup path.
            pass
        raise
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="Repeat to override the default two-profile production suite.",
    )
