...
from lyric_aligner.review.semantic_sensitive import (
    SCHEMA_VERSION as SENSITIVE_SCHEMA_VERSION,
...
class V4DisplayTextPolicyEndToEndTests(unittest.TestCase):
    ...
    def display_command(
        ...
        out_dir: Path,
        *,
        legacy_without_semantic_sensitive: bool = True,
    ) -> list[str]:
        command = [
            sys.executable,
        ...
        if legacy_without_semantic_sensitive:
            command.append("--legacy-allow-no-semantic-sensitive-review")
        return command

    def test_new_display_stage_requires_semantic_sensitive_review(self):
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            ...
            result = run_command(
                self.display_command(
                    fixture,
                    root / "display",
                    legacy_without_semantic_sensitive=False,
                )
            )
            self.assertNotEqual(result.returncode, 0)
            self.assertIn(
                "require --semantic-sensitive-pack",
                result.stderr,
            )
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            blocked_release = run_command(
                [
                    ...
                    str(out_dir / "FINAL.render.artifact.json"),
                    "--out-manifest",
                    str(out_dir / "FINAL.release.blocked.json"),
                ]
            )
            self.assertNotEqual(blocked_release.returncode, 0)
            self.assertIn(
                "lacks completed semantic-sensitive review",
                blocked_release.stderr,
            )

    def test_semantic_sensitive_review_is_bound_into_formal_final_render(self):
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            fixture = self.build_fixture(root)

            policy = json.loads(
                fixture["policy_path"].read_text(encoding="utf-8")
            )
            policy["mask_profile"] = "none"
            fixture["policy_path"].write_text(
                json.dumps(policy, ensure_ascii=False),
                encoding="utf-8",
            )

            pack_path = root / "sensitive-pack.json"
            pack = build_semantic_sensitive_pack(fixture["source_srt"])
            pack_path.write_text(
                json.dumps(pack, ensure_ascii=False, indent=2) + "\n",
                encoding="utf-8",
            )
            self.assertEqual(pack["required_decision_cue_numbers"], [2])

            review_path = root / "sensitive-review.json"
            review = {
                "schema_version": SENSITIVE_SCHEMA_VERSION,
                ...
                "scanned_cue_count": len(fixture["source_cues"]),
                "decisions": [
                    {
                        "cue_number": 2,
                        "timing": "00:00:01,000 --> 00:00:01,900",
                        "expected_text": "placeholder fuck token",
                        "action": "mask",
                        "mask_terms": [{"text": "fuck", "occurrence": 1}],
                        "reason": "synthetic semantic profanity decision",
                        "confidence": "high",
                    }
                ],
            }
            ...
            command.extend(
                [
                    "--semantic-sensitive-pack",
                    str(pack_path),
                    "--semantic-sensitive-review",
                    str(review_path),
                ]
            )
            ...
            qa = json.loads(
                (out_dir / "FINAL.qa.json").read_text(encoding="utf-8")
            )
            self.assertTrue(qa["semantic_sensitive_review_applied"])
            self.assertTrue(qa["semantic_sensitive_full_scan_completed"])
            self.assertEqual(
                qa["semantic_sensitive_reviewer_model"],
                "synthetic-model",
            )
            self.assertEqual(qa["display_text_sensitive_mask_count"], 0)
            self.assertEqual(qa["semantic_sensitive_masked_cue_count"], 1)
            self.assertEqual(qa["semantic_sensitive_masked_term_count"], 1)
            ...
            with (out_dir / "FINAL.audit.csv").open(
                ...
                rows = list(csv.DictReader(handle))
            self.assertEqual(rows[1]["semantic_sensitive_action"], "mask")
            self.assertEqual(rows[1]["semantic_sensitive_mask_count"], "1")

            artifact = json.loads(
                (out_dir / "FINAL.render.artifact.json").read_text(
                    encoding="utf-8"
                )
            )
            self.assertTrue(
                artifact["normalized_config"]["semantic_sensitive_review_applied"]
            )
            self.assertEqual(
                artifact["normalized_config"]["semantic_sensitive_review_sha256"],
                sha256_file(review_path),
            )

            semantic_run, semantic_fusion, semantic_qa = (
                write_passing_semantic_sync_fixture(
                    manifest_path=fixture["manifest_path"],
                    final_srt=out_dir / "FINAL.srt",
                    final_report=out_dir / "FINAL.audit.csv",
                    out_dir=out_dir,
            ...
            required_release_path = out_dir / "FINAL.release.semantic-required.json"
            required_release = run_command(
                [
                    sys.executable,
                    str(ROOT / "scripts" / "v4_validate_release.py"),
                    "--task-manifest",
                    ...
                    str(out_dir / "FINAL.render.artifact.json"),
                    "--out-manifest",
                    str(required_release_path),
                ]
            )
            self.assertEqual(
                required_release.returncode,
                0,
                msg=required_release.stderr,
            )
