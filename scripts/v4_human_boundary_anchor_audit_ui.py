#!/usr/bin/env python3
"""Project's sole default listening/human-review UI: frozen UX 3.2.
...
def verify_ui32_core() -> None:
    """Extensions must not replace the confirmed playback/layout core."""
    lock = json.loads((REPOSITORY_ROOT / "references/ui32-core-lock.json").read_text(encoding="utf-8"))
    if AUDIT_UX_REVISION != "3.2" or hashlib.sha256(HTML.encode()).hexdigest() != lock["base_html_sha256"]:
        raise RuntimeError("UI 3.2 core differs from the confirmed frozen version")


def render_review_extension(*, timing_manifest: Mapping[str, Any] | None = None) -> str:
    """One shared 3.2 page; task fields extend it without another renderer/UI."""
    verify_ui32_core()
    invocation = "load().catch(e=>document.body.innerHTML='<h2 class=\"bad\">审听工具启动失败</h2><pre>'+String(e)+'</pre>');"
    if HTML.count(invocation) != 1:
        raise RuntimeError("UI 3.2 initialization hook changed")
    page = HTML.replace(invocation, "")
    page = page.replace('<audio id="audio" controls preload="metadata"></audio>',
                        '<div id="review32Fields"></div><audio id="audio" controls preload="metadata" style="width:100%"></audio>')
    boot = json.dumps(timing_manifest, ensure_ascii=False).replace("<", "\\u003c")
    script = Path(__file__).with_name("ui32_review.js").read_text(encoding="utf-8")
    return page + '\n<script>const UI32_TIMING_MANIFEST=' + boot + ';\n' + script + '\n</script>'
...
def make_handler(app: AnchorAuditApp):
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self) -> None:
            parsed = urlparse(self.path)
            if parsed.path == "/":
                raw = getattr(app, "html", HTML).encode("utf-8")
            ...
            if parsed.path == "/api/identity":
                self._json(getattr(app, "service_identity", {"ui_revision": AUDIT_UX_REVISION, "pack": str(app.pack)}))
                return
            if parsed.path == "/file":
...
def _bind_server(app: AnchorAuditApp, port: int) -> ThreadingHTTPServer:
    ...
    server = None
    try:
        server = ThreadingHTTPServer((HOST, port), make_handler(app), bind_and_activate=False)
        # Windows SO_REUSEADDR can silently serve two different packs on one port.
...
def main() -> int:
    ...
    print(f"Listening/review UI {AUDIT_UX_REVISION}: {url}")
    if not args.no_open:
