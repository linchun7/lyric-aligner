...
class TextRepairV2Tests(unittest.TestCase):
    ...
    def test_word_reflow_preserves_existing_leading_punctuation(self):
        for text, target in (
            ("wekeep 'em close", "we keep 'em close"),
            ("wekeep ’em close", "we keep ’em close"),
            ('wekeep "it" safe', 'we keep "it" safe'),
        ):
            with self.subTest(text=text):
                source = f"1\n00:00:01,000 --> 00:00:02,000\n{text}\n"
                output, report = repair_srt_text(source, canonical(target))
                self.assertEqual(parse_srt_text(output)[1][0].text, target)
                self.assertEqual(report["status"], "ready")
                self.assert_timeline_unchanged(source, output)

    def test_punctuation_preservation_does_not_restore_redundant_spaces(self):
        source = "1\n00:00:01,000 --> 00:00:02,000\nwe  keep 'em close\n"
        output, _ = repair_srt_text(source, canonical("we keep 'em close"))
        self.assertEqual(parse_srt_text(output)[1][0].text, "we keep 'em close")
        self.assert_timeline_unchanged(source, output)
