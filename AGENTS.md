...
- 当前开发入口以 [README.md](README.md)、[SKILL.md](SKILL.md) 和 [references/workflow.md](references/workflow.md) 为准；模块生产资格见 [module-lifecycle](references/module-lifecycle.md)。2026-09-11 已停止继续调当前 source-ASR+harmonic retrieval 与 English final-mix HuBERTFA timing family；Qwen/SOFA/HuBERTFA/STARS 权重已清理，除非有新的真实 production failure + 预注册实验 + untouched truth，不得自动下载/重装或恢复为 production。boundary-level timing 的下一阶段只走 [P1 shadow gate](references/boundary-promotion-p1.md)：blind review manifest 必须在人工 review 前绑定 pre-gold frozen selection hash + intended partition，原始 candidate-blind response 必须保留并参与 response→Gold 重算校验；development/calibration response 不得事后重标 blind/holdout；shadow PASS 仍不授予 production authority。旧 [next-stage Max handoff](references/next-stage-max-expected-loss-handoff-2026-09-07.md) 仅作历史研究记录。
