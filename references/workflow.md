...
所有试听和人工复核统一 [唯一默认 UI 3.2](listening-ui.md)：双击 `试听复核.cmd`，统一 CLI `scripts/v4_listen.py`。先绑定当前复核 pack，再启动；Safe/R5、Max、Gold、转场/ownership 都复用原 3.2 播放核心。旧页面仅 legacy 兼容，不另造 UI；字幕需直接显示，听不懂保留 unknown，作答和生产修改权限分开。

更新：2026-09-18
当前证据路径：`Standard / Smart stable floor -> Safe Final；Max 仅用于时间轴事故恢复`
当前用户路径：`Safe Final / 最低下限最终版`；异常路径：`Max Recovery`。内部 Best-Safe/Max Release 保持既有资格，不是强制逐级升级链。

正常生产先用 `v4_review_safe_final.py scan --plan PLAN.json --out CANDIDATES.json` 扫描当前已审定 floor，再由 GPT 审定、复用 canonical splice 物化。Gap Rescue 仅检测、审定和验证，不写新 cue；音频默认不整首重跑，仅必要局部使用 large-v3-turbo。文字与 QA 收敛后，每个真实任务还必须执行 `sensitive-pack -> GPT 全量语义敏感词审查 -> sensitive-finalize`，只有该 mask-only finalizer 输出才是正式 Safe Final。完整契约集中在 [Safe Final review](safe-final-review.md)，不新增产品模式、selector 或 UI。
...
最低保底成品在现有Smart/安全floor之后，按[已审定canonical字符片段契约](reviewed-canonical-splices.md)保留有充分证据的文字增量。先`v4_apply_reviewed_text_splices.py --plan PLAN.json --inspect`查看当前完整前后字幕与实际空档，再以绑定review执行到新目录；未确认部分保持floor与review。这是同一最低保底流程的具体审定入口，不是新证据模式，不改变Smart/Best-Safe/Max自动authority。
...
5. 使用最便宜且足够的证据：`Standard / Smart stable floor -> Safe Final；Max 仅用于时间轴事故恢复`。
6. 证据不足时 fail closed 到 review，不降低阈值、不静默回退 legacy。
7. 输入、配置、artifact、review、release 都必须可追溯，不覆盖原件。
8. 敏感词属于最终发布层语义判断：词表/正则只负责提示候选，不能自动写回；GPT 必须完整扫描冻结后的最终文字并结合上下文决定 KEEP/MASK/REVIEW，程序只验证绑定并物化高置信 MASK。

### 1.1 最终语义敏感词 gate（真实任务必跑）

文字、Gap review 与 QA 收敛后，把准备发布的完整 SRT 冻结成 review pack：
...
GPT 必须读取 pack 中的全部 cues，而不只看机械候选；候选 hint 只用于提醒 strong profanity、语境相关词和已经预先 mask 的文本。GPT 输出 task-bound review：`reviewer_type=model`、真实 `reviewer_model`、`full_scan_completed=true`、`scanned_cue_count=pack.cue_count`，并对全部 `required_decision_cue_numbers` 明确给出 KEEP/MASK/REVIEW。发现 hint 未覆盖的问题时，GPT 可以主动增加 MASK/REVIEW 决定。MASK 只提交原字幕中精确存在的 `mask_terms`，不提供自由替换歌词；程序统一物化为“首字符 + `*`”。
...
finalizer 会重建 pack、核对 source SRT/hash、完整候选 ledger、expected text/timing 与模型身份；任一 REVIEW、漏审、stale 输入、mask term 不存在/重叠或输出覆盖旧文件都会 fail closed。成功时 cue count/number/start/end 必须与输入 SRT 完全一致，只允许 mask-only viewer text 变化。Best-Safe/旧 display policy 的 `strong_profanity_v1` 仍可显式用于历史兼容，但未来正常任务默认不机械写回敏感词。
...
1. **自动吸收**：不会破坏 Smart timing/cue ownership 的高确定性文字纠错；canonical 显式 `*` mask 恢复；task-bound normalized-equivalent display；以及已经绑定具体 Smart boundary 与独立真值的 timing promotion。`strong_profanity_v1` 不再属于未来正常任务的默认自动吸收项。
...
- `BEST_SAFE/QA.json::publish_ready=true` 至少要求：Smart topology exact；`unsupported_timing_change_count=0`；全部 timing truth PASS；文字/display verifier PASS。该状态只表示 pre-release selector 通过；正式 Safe Final 仍必须经过最终 GPT semantic-sensitive gate。该状态不授予 Max Release authority。
...
已获得 production authority 后，新 production release 必须经过最终语义敏感词 display materialization；如同时需要平台文字显示修订或极端未知 end 的 shorten-only presentation trim，也在同一阶段完成：

```text
v4_apply_display_policy.py
```

display layer 不回写 canonical lyric，不移动 cue start，不改变 occurrence/track/canonical identity。新 production 调用必须提供 `semantic-sensitive-pack/review`；模型 override 必须 task-bound 且绑定 expected text。旧强脏词自动规则仅供显式 legacy 兼容，正常发布不得与 semantic-sensitive model review 同时启用。
