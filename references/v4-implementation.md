...
`v4_human_boundary_anchor_audit_ui.py` 保留 `HTML` 原字节，`ui32-core-lock.json` 校验其 SHA；服务绑定补 Windows 独占端口，避免两个复核包同端口共存。`v4_listen.py` 负责包类型/默认指针/实例身份；`ui32_review.js` 只在同一 HTML 的固定扩展点显示对照字段或旧 P1 导出。`review/listening.py` 保存逐次原生作答、核对输入、做相同窗口的配对描述统计，不调用生产物化。旧 gap 默认生成入口废弃，P1 默认 renderer 指向 3.2，legacy 函数/历史产物保留。UX 3.2 与旧 response 3.0 分开，Gold/selector authority 不变。详见 [listening-ui](listening-ui.md)。
...
```text
Standard / Smart stable floor -> Pro（按需局部证据）
Max 仅供 Max Recovery 事故恢复
```

用户交付路径：

```text
Safe Final / 最低下限最终版：Standard/Smart floor -> GPT lexical review -> QA -> GPT semantic-sensitive full scan -> mask-only finalizer
Max Recovery（仅整体 timeline 明显损坏）
```
...
P1 boundary-promotion shadow 在 evaluation 层补齐 pre-gold selector/partition freeze 与 blind/holdout gate：`timing_decision_pack.py` 先锁定 population/candidate positions，`boundary_promotion_shadow.py` 冻结 selector id/revision/code SHA、逐 boundary KEEP/PROMOTE、独立 evidence family/correlation group 与 gate policy；Smart==candidate 的 unchanged control 不能被计作 promotion。冻结后，`v4_build_timing_decision_review.py --boundary-promotion-selection ... --boundary-promotion-partition ...` 必须把 exact `selection_payload_sha256` 与 intended partition 一起写入 candidate-blind review manifest；两项 P1 参数成对出现，普通 timing review 保持原兼容行为。人工 response 绑定该 manifest hash，Gold ingestion 的 `partition` 必须与 manifest frozen partition 一致，并继承 manifest hash + selection hash + partition。P1 evaluate 同时读取 selection / review manifest / raw response / Gold：先验证 manifest 绑定 frozen selection/partition，再用 `validate_review_response()` 重算期望 Gold 并与实际 Gold 逐字段比对，防止旧 Gold 事后换绑 selector、development response 重标 blind，或 response 后手工改 boundary/partition。selection 漏记、重复、stale、rehash 后 identity/count 漂移、unchanged-control promotion 或四方 lineage 不一致全部 fail closed。shadow gate 仍要求实际 promotion 数/track 覆盖、catastrophic harm、P90、mean/track-equal gain 与 track bootstrap 同时达标；无论 PASS/BLOCK，当前实现固定 `production_authority_granted=false` / `production_writeback_permitted=false`，不生成 Best-Safe timing promotion。该 hash 链证明仓库内 artifact 一致性，不等于外部可信时间戳；真正 blind campaign 仍必须保存原始 response。该模块只测量 selector 是否可能优于 Smart，不改变下述 production authority graph。
