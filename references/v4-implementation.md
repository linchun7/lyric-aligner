...
### Smart v1.2.10 / Pro v1.2.5 split-line and budget correction
...
`selective_policy.py` consumes `timing_high_value_pro_candidate_positions` as a first-budget key while retaining all actionable suspicions in the independent manual queue. `asr_executor.py` treats `asr_language_hint=auto` as no concrete override, allowing canonical-local routing only when an explicit source language makes the script unambiguous. Mixed/unknown/source-auto continues backend auto-detection. Pro advances to v1.2.5; automatic text/timing mutation remains disabled.
