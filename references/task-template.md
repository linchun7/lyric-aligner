...
更新：2026-09-18
当前用户产品路径：`Safe Final / 最低下限最终版`；异常恢复路径：`Max Recovery`
当前 Max 主线版本：`4.0.0a20`

本文件只描述当前生产入口。Standard / Smart / Pro / Max 是内部能力层，不是用户必须逐级选择的四个成品版本；Balanced / Fluent 已撤出当前产品层。历史 v3.9/`redo_karaoke_pipeline.py` 仍保留用于回归与历史兼容，但不再是新任务默认生产路径。

## 1. 先选择稳定 floor

- **一般中文**：优先 Standard stable floor；editor/Jianying timing 可信时只修 canonical 文字，不读 audio，不改 cue timing。重复副歌、拆句/合句、occurrence 或 ownership 不确定时再用 Smart。
- **一般韩文 / K-Pop**：优先 Smart stable floor，再进入 Safe Final lexical review。重点识别 pseudo-English、韩英混唱、真实英文和重复 occurrence；Latin token 不能因为歌曲是韩文就自动替换为 Hangul。
- **Pro**：只处理少量 unresolved cue，提供 bounded acoustic / ASR / forced evidence；不是必跑层，不自动写 timing/text。
- **Max Recovery**：整体 timeline 不可信、复杂 cut/overlap/reorder 或大范围 occurrence 损坏时才使用 Full V4。

外文、韩文、日文不是自动进入 Max 的条件；最终正常交付统一收敛为 Safe Final。无论使用 Standard 还是 Smart，文字与 QA 收敛后都必须再经过 GPT 全量语义敏感词审查和 mask-only finalizer；这一步属于发布层，不改变 canonical truth、cue 数、分段或 timing。
...
当前 `main` 的 Smart selector 为 v1.2.11。普通“大部分时间轴正确”的任务先走 Smart；是否能结束任务仍由该次 report 的 `status/pro_escalation_required` 和后续 release gate 决定，不因版本号自动视为发布通过。
...
当前 Pro 为 v1.2.7，只处理 Smart unresolved 的 bounded regions。
