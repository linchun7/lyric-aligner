...
## 2026-09-18：Safe Final 最终语义敏感词 gate

正常发布链在歌词文字/归属审定、Gap review 与 QA 之后新增一个薄的 mandatory display gate：`v4_review_safe_final.py sensitive-pack` 冻结完整 pre-release SRT 与可选 song list，机械规则只生成 strong-profanity / context-required / pre-masked 提示，明确 `automatic_replacement_allowed=false`。GPT 必须读取全部 cues，绑定 pack/source SRT hash，声明真实模型身份、`full_scan_completed=true` 且 `scanned_cue_count=pack.cue_count`，并对所有提示 cue 给出 KEEP/MASK/REVIEW；未提示但语境上有风险的 cue 允许由模型主动追加 MASK/REVIEW。

`sensitive-finalize` 不接受自由 replacement text。MASK 只提交原 `expected_text` 中 exact `{text,occurrence}`，程序统一物化为首字符加 `*`；KEEP/MASK 必须 high-confidence，任一 REVIEW、候选漏审、stale pack/source、expected text/timing 不符、term 缺失/重叠或输出覆盖旧文件都 fail closed。成功时 cue count/number/start/end 与输入 SRT 全等，只生成 viewer-facing mask 和独立 audit，不授予 canonical/timing/segmentation authority。为避免模型审查前已经丢失原文，Best-Safe future default `mask_profile` 从 `strong_profanity_v1` 改为 `none`；旧 profile 与旧 display-policy 自动 mask 仅保留显式历史兼容。没有新增产品模式、ASR、timing authority 或 UI。

## 2026-09-18：当前文档与仓库隐私边界收口

将 README、SKILL、task/prompt template、production requirements、runtime guide、implementation 和 multilingual roadmap 统一到同一口径：用户正常交付只有 Safe Final / 最低下限最终版；Max Recovery 仅用于整体 timeline 事故。Standard / Smart / Pro / Max 明确降为内部能力层，Balanced / Fluent 不恢复。默认语言路由固定为“一般中文 Standard floor、复杂中文再 Smart；一般韩文/K-Pop Smart floor + lexical review；Pro 仅局部证据”。

同步强化公开仓库边界：通用仓库只提交代码、通用文档和脱敏 synthetic fixtures；真实歌词/字幕/音频、任务级 Gold/review/QA、绝对本地路径、用户名、凭据及本地编排证据保持 ignored/untracked。任务级状态在当前状态页只保留通用能力结论，不再写本地输出路径。历史 release manifest 中冻结的旧 evidence-path 字符串继续保留作 lineage，不将其解释为当前产品路径。

## 2026-09-18：Safe Final 最终 Review 与窄 Gap Rescue 审定

正常交付收口为 Standard/Smart stable floor → GPT 语义/发音/occurrence 审定 → 原有 canonical splice → QA → Safe Final。Max Recovery 只用于严重时间轴事故；不是新增 mode，也不是继续开发 selector。新增一个小的只读 `review/safe_final_review.py` 和 CLI，复用既有 parser、韩文发音提示、严格双射简繁比较，保留完整前后文、实际空档、真实英文提示和重复 occurrence；重复歌词本身不再把已解决英文分段重新排入 review。简繁比较不改写 canonical display。

通用文字物化器补齐报告 `output_srt_sha256` 与所绑定 baseline 的交叉核对、同源有序且不重叠的 canonical segments、落盘前所有输入/审定/观察的 hash 复核；QA 始终保留冻结的 floor identity。inspector、scanner、materializer 共用基线 reader，文件哈希复用流式实现，去掉重复整文件 hash；真正 Standard 报告和 untimed canonical 复用同一物化器，不要求为了修字升级 Smart/Max。未改变 schema `reviewed-canonical-splices-1.0` 的字段；旧缺失/错绑报告现在 fail closed，不重写历史输入。

Gap Rescue 只接受完整 exact baseline 锚点之间真实空档与连续未覆盖 canonical 行作为候选；已有较早长 cue 或其他 mapping 占用时不产生重复 gap。review/verifier 区分漏字幕、未唱、间奏、cut、overlap、版本和 occurrence 风险，重建 exact pack，拒绝 stale/hash/重复遗漏决定/布尔数值混淆/越界时间/伪造 authority。即使文字和时间区间填写完整，仍固定 `production_writeback_permitted=false`、`subtitle_mutation_count=0`；尚未接入新增 cue 的原始 final-mix timing truth，不新增 Gap writer。最小后续输入、实现、验收与回退见 [Safe Final review](safe-final-review.md)。

验证覆盖上一版两个输入漏洞复现、中文错字/漏多字、韩英 partial splice、既有韩文修复继承、canonical 版本/重复/时间/lineage 拒绝，以及代表性历史/当前任务的兼容回放。它们是兼容/契约回归，不是新的 untouched blind 或总体正确率。最终本地 Python 3.14.6 完整 1987 tests / OK、定向 56 tests / OK，全部9项检查通过，测试期间源码 hash 不变。任务级 partial-text 与 Gap Rescue 结果只保存在本地私有输出，不进入通用仓库；不声称整条解决或独立 Gold 收益。

删除/冻结决定：删除重复基线解析及整文件 hash 路径、精简 README 的重复历史说明；没有证据满足删除条件的生产依赖不删。特别纠正旧 Smart wrapper 仅历史的文档表述：v1.2.11 实际调用 v1.2.10，其源码不能误删。Balanced/Fluent、不受独立真值支持的 timing promotion、整首 Max 自动吸收和已停止的实验 backend 继续冻结；UI 3.2 和 production tag 不动。回退本轮通用代码使用本条前的源码 commit；保留所有任务级输入、审定、Gold、历史 final 与必要 lineage，不用旧 floor 覆盖已确认的生产修复。


## 2026-09-18：架构收敛，撤出 Balanced / Fluent 实验产品层

对当前未提交工作做生产导向收敛：Balanced / Fluent 没有新的独立韩/日 Gold 或 untouched blind 证明其真实净正确率，且与已经验证有效的 Smart/Best-Safe floor + 已审定局部文字增量路线重叠，因此不进入下一版当前源码。删除其未提交 product pipeline、CLI、专属文档和测试；历史 output/private 证据不删除，不改写旧 artifact。

保留的净收益包括：Text Repair 已有标点/横向空白确定性修复、Smart v1.2.11 / Best-Safe 1.1 timing/topology floor、任务绑定的 reviewed canonical character splice、安全增量交付，以及唯一默认试听/复核 UI 3.2。生产证据路径、产品 authority、算法版本和 `prod-v4.0.0a20-best-safe-v1.1.0-20260912` freeze 均未因此改变。

## 2026-09-13：唯一默认试听/复核 UI 3.2

锁定用户确认的 `v4_human_boundary_anchor_audit_ui.py` 原 HTML SHA，原布局/时间轴/粗细调/A-B/倍速保持。新增统一 `v4_listen.py` 和 Windows 一键入口，按 pack + 当前代码 SHA 复用实例；修复 Windows 同端口重复绑定导致旧/新包混用的问题。P1 默认页面改由同一 3.2 渲染，原导出/ingest 校验不变；旧 gap CLI 需显式 legacy flag，其余历史 UI 不再作为默认。

验收覆盖真实 WAV、1440/760px 布局、保存/刷新、P1 导出→原 ingest、未保存修改拦截、同包复用、异包端口拒绝与 legacy 默认拒绝；compile、skill、privacy、环境及 dirty docs-contract 当时均通过。测试只证明 UI/协议兼容，不计入真实 Gold 或字幕成品收益。

一键入口最终实测另修 Windows PowerShell 5 的子进程状态误报：venv launcher 重定向后 timed wait 会返回空 ExitCode，错误提示成功复用失败；先保留进程 native handle 后取得真实 0/非零退出码。相同环境修前复现空值、修后 0，根目录 `.cmd` 实际复用退出 0、stderr 为空。此 PS 修正后验证实际启动路径，不因非 Python 改动重复整套媒体测试。

字幕对照仅同页字段扩展：直接显示文字和差异，逐版判断歌词/归属/明显时间错误，保留未知与语言能力。原始作答追加保存，不自动改 SRT 或授予 Gold。配对统计使用共同时间窗口，分母和 unknown 明确，无作答比率为 null。未取得新合格韩/日人工 Gold 时不能声称语义正确率收益；Safe/Max/DP/ownership 策略不因 UI 改动而变化。冻结点、旧入口清单和验收限制见 [listening-ui](listening-ui.md)。

## 2026-09-13：65条残留逐条取舍、通用已审定字符修复与交付 .5

在 `.4` 上新增采用18处（KPOP110 11、KPOP200 7），5条整条文字审定、13条局部改善仍review；47个输入cue不变。原65条确定残留减少7条至58条，其中两条只是清除目标伪英文、仍有其他文字/边界问题；高疑似308条不变。29个无歌词提示扩窗覆盖38个强候选，继续使用同一保留ASR；无效输出不授权修改。KPOP110/574实际前邻空档6100ms推翻旧lips连续句推断；296仅修后半，549仅修一个确定词，不再把整句不确定作为局部收益的障碍。

将既有任务级canonical字符拼接能力提取为`review/canonical_splices.py`与`v4_apply_reviewed_text_splices.py`，复用既有parser/renderer，支持任意曲目/字符，不内置这些cue号和歌词。输入/审定哈希绑定、精确原字符范围、外部观察、措辞/归属双理由、完整keep台账、空档显示和父floor回退可复用于未来任务；词义判断仍是具体审定，未获未知歌曲自动替换资格。旧R2/R3/R4的150处采用回放，11组成品逐字节一致；不是新增正确数或独立Gold。五批3,286条结构/时间和其他三批字节保持，未扩张Max或调DP。契约见 [通用字符片段入口](reviewed-canonical-splices.md)，成品见 [安全增量交付](smart-safe-incremental-delivery.md)。

## 2026-09-13：KPOP全量伪英文复核与任务交付 .4

以 `.3` 当前成品为父floor，逐条覆盖KPOP110 639 + KPOP200 507 = 1,146个实际cue；用户预期1,147与原编号跳过129的差异明确记录，不补号。结合canonical、相邻cue、Smart已有映射和历史失败审计作四类判断；另为220个候选取124个局部窗口，并对6个原窗口作一次无歌词的韩语提示复查。复用保留的同一ASR、parser与renderer，不改生产核心、Max或ownership/DP。

新增采用105条（44 + 61），其中56条整条文字审定、49条局部改善；既有R3全部修复保留，其他三批逐字节保持。任务级recipe冻结每条输入、canonical字符位置、原位替换、实际观察及理由，保持全部时间和cue分段。韩文与英语近音的语义/发音判断由当前大模型参与，原始音频观察和canonical是外部依据，不能把相关解码称为独立Gold。KPOP200/506恢复빛이나는솔로，KPOP110多处词内cue分界原样保留。

最终仍有65条确定错误类型但完整修法未定、308条高疑似；两批分别176/197条残留。完整1,146条台账、105条改前改后及373条残留清单随产物交付；机械完整性检查不能冒充真实净正确条数，人工编辑时间未测。没有另开路线或调阈值，未取得新自动化策略authority。见 [当前成品与证据边界](smart-safe-incremental-delivery.md)。

## 2026-09-13：同一最低保底路线的韩语伪英文逐条修复

任务交付修订 `2026-09-13.3` 以 `.2` 的五批最终 SRT 为父 floor，扫描932个风险候选并选62条作逐条文字与归属判断。复用已有403条 Korean ASR 缓存，补充78个局部窗口；短窗口的片尾幻觉和重复不计作支持，只补查一次包含相邻 cue 的窗口。旧 V3 的整批 FAIL 未改写，也未授予其67条提案批量生产资格。

新增采用44处（KPOP110 21、KPOP200 13、mixed130 10），18条因证据或归属不足保持。43条整段文字审定、1条仅局部改善分别标记；KPOP110/296的整行提案因漏掉前一句尾词而拒绝。使用当前大模型的韩文/发音/上下文判断审定具体 canonical 词段，既有 parser 和 renderer 物化明确白名单，不增加生产通用框架，不调 ownership/DP。仅认领本次具体文字判断，不冒充独立 Gold、可靠人工听审或机器 timing authority。五批编号、时间、cue数量与分段完全保持，继承前一修订全部收益；原始产物及证据保留。见 [安全增量交付](smart-safe-incremental-delivery.md)。

## 2026-09-13：Smart 安全增量成品交付（前一修订 .2）

目标改为逐条交付已审定的安全增量，不要求完成49组听写或统计实验才采用一个具体修复。原版 UI 3.2 恢复完整试听控件并显示两版字幕，用户通过选择作答；收到3组记录，多次保存不算独立投票。用户明确提示语言理解有限，因此所有记录按有不确定性的辅助证据保存，原始响应不改写为 Gold。

先交付一个原最强候选，再从全部1,467条 Smart review 中检查单 cue 候选，追加9个局部音频窗口，合计106.6秒、推理86.45秒；仍使用本机保留权重和既有 executor。新增接受一个原位单词修复，其他候选因转写冲突、无关输出、归属或版本不确定保留 Smart。五批任务交付修订 `2026-09-13.2` 累计接受两条词义修复，保留旧 floor 的安全改动；3,286 cues 的时间轴与拓扑不变。

第一条经现有 Best-Safe 文字 verifier；第二条为绑定单个任务/cue/canonical 位置的局部音频文字审定，仅替换原位一个词，经现有 canonical parser、Pro word matcher 和 SRT renderer 校验及物化，不宣称通过未满足的无音频整段锚点 gate。所有原始产物和旧 authority 保持；没有新通用 selector、模型或 DP 门槛，未移动 production tag。工程与证据完整性不等于独立 Gold 正确率。见 [安全增量交付](smart-safe-incremental-delivery.md)。

## 2026-09-13：49组局部音频取证与无音频路线收尾

冻结49组历史 development 窗口，以现有 Faster-Whisper executor 和保留的 large-v3-turbo 权重完成无提示机器取证；音频合计582.365秒、CPU推理453.171秒。未向解码器提供候选、Smart、canonical 或语言提示，未下载其他权重，未调 ownership/DP 阈值。原始 response、clip/model/source hashes 与候选冻结链全部保留。机器观察不冒充人工 blind；原生人工响应仍为0，Gold为0，未测得成品正确率及人工修复量收益。

预注册比较中，A0的22个区域有14个更接近Smart、4个更接近候选，其余无观察/持平/仅展示差异；这是ASR代理距离，不是正确率。事后复用现有 Pro word-span matcher找到3个 cue 的候选完整词序支持，其中1个属于单 cue 候选，保留为优先听审项；不授予写回。正式停止本轮无音频词义/归属重建开发，保留上一轮已验证的两处排版修复，不扩张 Max。新实验与听审页面只在本地产物目录，生产代码/schema/authority 无新增改动。见 [完整收尾边界](noaudio-floor-trial-2026-09-12.md)。

## 2026-09-12：无音频成品下限实验与标点空格修复

本地修复分支保留一个共享排版函数的确定性修复：已有英文词边界的空格与标点共处时，在原位置压缩横向空白，不再把空格统一移到标点后面。普通多余空格整理、拆词修复和换行拒绝逻辑继续生效。生产冻结 tag、Smart facade、Max / Best-Safe authority 和 schema 不变；未发布新生产版本。

五批共 3,286 cues 的 Smart 基线逐字节复现。现有 word DP 产生 22 个区域、47 个实际变化 cue；19 个区域触发现有保护或与字符归属分配矛盾，这不是 19 个已标注错误。字符分配对照没有证明新的安全词义/归属收益，未进入生产。最终排版修复对照只改变两处横向空白，所有 cue 的非横向空白字符、时间轴、拓扑和 review 集合相同。人工 blind 尚无原生响应，不宣称成品正确率或 Max 整体优势得到验证。

回归覆盖表示省略的撇号、弯引号、开引号与多余空格；修复前失败已保留。Python 3.12 全量 1,910 项测试通过，4 项可选测试跳过；这些是工程回归证据，不是成品正确率证明。实验参数、被否决的中间方案、基线、候选和盲标待办留在本地实验目录。决策与复现边界见 [无音频下限实验](noaudio-floor-trial-2026-09-12.md)。
