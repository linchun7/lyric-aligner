...
状态：development / backward-compatible extension  
日期：2026-08-18
...
P7 single-job protocol remains valid for compatibility/debugging.
...
## 7. Model identity and multilingual adapters

Batch protocol assumes the configured backend/model identity is auditable for the entire invocation. A future WhisperX adapter must not silently load different language checkpoints while reporting one misleading checkpoint ID.
...
The first real reference adapter should prefer same-language / same-model batches until real multilingual model lineage is explicitly implemented.
...
Fake-runner and real-subprocess tests may prove one process handles multiple jobs and that exact response-set validation works. They do not prove real model throughput or singing accuracy.
