...
2026-09-12 Best-Safe 1.1 使用 `scripts/v4_build_best_safe.py` 作为最低风险产品 selector。它不新增声学 authority，也不改写 Smart/Max：必须显式提供原始 Smart SRT/report、canonical lyrics、Max SRT/audit、逐曲 semantic sync、song list 与 transition authorization；Max/semantic/transition 在 1.1 默认只提供候选/诊断，不授予 whole-track timing/topology 写权限。Best-Safe 默认完整继承 Smart cue 数、顺序、start/end；可选 task-bound text adjudication、`strong_profanity_v1` display mask、normalized-equivalent display overrides、`--timing-truth` 与 `--timing-promotions`。text adjudication 必须绑定 task fingerprint、Smart SRT SHA、Smart report SHA 和全部 canonical lyric SHA；启用后 proposal + keep-Smart 必须恰好覆盖 Smart report 的全部 `review` cue，漏记、重复覆盖或非-review 引用一律失败；不得携带 free-form replacement/timing，且多 cue 物化除了完整 text-region policy 外还必须保持 cue ownership：无 timing authority 时不得插入、删除或跨 cue 搬移 Latin token。`--timing-truth` schema=`best-safe-timing-truth-1.0`，必须绑定 task fingerprint + Smart SRT SHA；当前 authority 仅接受 `human_truth`。`--timing-promotions` schema=`best-safe-timing-promotions-1.0`，每项必须绑定 `track_ordinal + source_cue_number + start/end + expected_smart_ms + truth_id`，且 promoted value 必须落在对应 human truth tolerance 内；track semantic PASS、同源模型一致或 transition action 均不能替代该 authority。构建后 `topology_exact=true`、`unsupported_timing_change_count=0`、全部 timing truth PASS 才能 `publish_ready=true`。`--lyrics-dir`、truth/promotion 与其他输入均为受保护输入；输出树不得污染任一输入。该状态只授予 Best-Safe 当前证据合同，不等于 Max Release authority。
