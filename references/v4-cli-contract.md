...
唯一默认试听/复核：`python scripts/v4_listen.py --pack-dir PACK --set-default`；之后一键 `试听复核.cmd` 或 `python scripts/v4_listen.py`。`--check` 只校验，`--report NEW_JSON` 只输出对照描述统计。默认 localhost:8765，不静默换端口；仅复用 pack/代码 SHA 全部一致的既有实例。支持原 anchor、P1 manifest/materialization、字幕对照 pack；UX 固定 3.2，旧 anchor 协议 3.0 不变。完整参数和废弃入口见 [listening-ui](listening-ui.md)。

2026-09-12 Best-Safe 1.1 使用 `scripts/v4_build_best_safe.py` 作为最低风险产品 selector。它不新增声学 authority，也不改写 Smart/Max：必须显式提供原始 Smart SRT/report、canonical lyrics、Max SRT/audit、逐曲 semantic sync、song list 与 transition authorization；Max/semantic/transition 在 1.1 默认只提供候选/诊断，不授予 whole-track timing/topology 写权限。Best-Safe 默认完整继承 Smart cue 数、顺序、start/end；可选 task-bound text adjudication、显式历史兼容 `strong_profanity_v1` display mask（未来正常任务默认 `none`）、normalized-equivalent display overrides、`--timing-truth` 与 `--timing-promotions`。text adjudication 必须绑定 task fingerprint、Smart SRT SHA、Smart report SHA 和全部 canonical lyric SHA；启用后 proposal + keep-Smart 必须恰好覆盖 Smart report 的全部 `review` cue，漏记、重复覆盖或非-review 引用一律失败；不得携带 free-form replacement/timing，且多 cue 物化除了完整 text-region policy 外还必须保持 cue ownership：无 timing authority 时不得插入、删除或跨 cue 搬移 Latin token。`--timing-truth` schema=`best-safe-timing-truth-1.0`，必须绑定 task fingerprint + Smart SRT SHA；当前 authority 仅接受 `human_truth`。`--timing-promotions` schema=`best-safe-timing-promotions-1.0`，每项必须绑定 `track_ordinal + source_cue_number + start/end + expected_smart_ms + truth_id`，且 promoted value 必须落在对应 human truth tolerance 内；track semantic PASS、同源模型一致或 transition action 均不能替代该 authority。构建后 `topology_exact=true`、`unsupported_timing_change_count=0`、全部 timing truth PASS 才能 `publish_ready=true`；该状态是 pre-release selector 资格，未来正常任务还必须经过最终 GPT semantic-sensitive gate 才能称为正式 Safe Final。`--lyrics-dir`、truth/promotion 与其他输入均为受保护输入；输出树不得污染任一输入。该状态只授予 Best-Safe 当前证据合同，不等于 Max Release authority。

2026-09-12 P1 boundary-promotion shadow 新增 `scripts/v4_boundary_promotion_shadow.py`，**只做评测、不生成 production timing promotion**。`freeze-selection` 必须读取已经 pre-gold 锁定的 timing decision pack 与 schema=`boundary-promotion-machine-decisions-1.0` 的完整机器 decision ledger；每个 frozen case 必须恰好一个 `keep_smart|promote_candidate`，并绑定 frozen Smart/candidate ms、selector id/revision/code SHA。promotion 还必须声明 `independent_timing_evidence=true`，同时提供 candidate/evidence family 与不同 correlation group、evidence SHA 和 reason code；漏记、重复、stale、同 correlation group 自证、rehash 后 identity/count 漂移以及 Smart==candidate 的 unchanged-control promotion 一律 fail closed。P1 的 candidate-blind review 必须通过 `v4_build_timing_decision_review.py --boundary-promotion-selection <selection.json> --boundary-promotion-partition <partition>` 把 exact `selection_payload_sha256` 与 intended partition 一起写进 review manifest；两个参数必须成对出现，普通非 P1 review 不受影响。人工 response 继续只绑定 manifest hash并隐藏 candidate position，`v4_ingest_timing_decision_review.py --partition` 必须与 manifest frozen partition 完全一致，生成的 Gold 会继承 manifest hash、P1 selection hash 与 frozen partition，因此 development/calibration response 不能事后重标为 blind/holdout。`evaluate` 现在必须同时提供 `--selection`、`--review-manifest`、`--review-response` 与 `--gold`；评估会验证 manifest 确实绑定 frozen selection/partition，并重新从 raw response 计算期望 Gold，逐字段核对 Gold，因此旧 Gold 不能事后换绑到新 selector，Gold 也不能在 response 之后手工改 boundary/partition。只有预先冻结为 `blind|holdout` 的 review partition 可通过 shadow gate，development/calibration 永远只作诊断。默认 gate 与 selection 一起冻结，要求至少24个有效 truth、4个独立 track、8个实际 promotion、promotion 覆盖4个 track、0个新 >500ms catastrophic error、>100ms harmful rate<=5%、P90 不退化、平均及 track-equal gain 为正、track bootstrap 95% CI 下界非负。无论 PASS/BLOCK，输出固定 `production_authority_granted=false` 和 `production_writeback_permitted=false`；P1 CLI 不接受 Best-Safe 输出目录，也不能生成 `best-safe-timing-promotions-1.0`。selection/evaluation JSON 统一经过 shared `validate_separate_artifact_paths`，evaluate 的 pack/selection/review-manifest/review-response/gold 都是受保护输入；目标文件必须事先不存在，并使用 shared `atomic_write_json` 原子物化。该 hash 链不是外部可信时间戳，真正 blind campaign 仍必须保留原始 response 且不得事后重写整套 artifact。完整协议见 [boundary-promotion-p1.md](boundary-promotion-p1.md)。
...
试听默认统一 [3.2](listening-ui.md)：`python scripts/v4_listen.py --pack-dir <pack> --set-default`，或双击 `试听复核.cmd`。旧 gap 页面只作兼容：`python scripts/v4_build_gap_ab_review.py --legacy-ui --job <gap-job.json> --output-dir <new-dir> --expand-case <id>`；不带 `--legacy-ui` 拒绝生成。历史扩大窗口/lock/导出协议不变，不作为新任务入口。
...
### `v4_review_safe_final.py sensitive-pack / sensitive-finalize`

所有真实 Safe Final 在最终文字与普通 QA 收敛后必须运行 `sensitive-pack`，冻结完整 pre-release SRT（可选绑定 song list）并生成 `semantic-sensitive-review-1.0` pack。pack 必须包含完整 cue 序列；strong profanity、语境相关词和 pre-masked 文本只作为 `automatic_replacement_allowed=false` 的提示。模型 review 必须绑定 pack/source SRT SHA，声明 `reviewer_type=model`、非空 `reviewer_model`、`full_scan_completed=true`，且 `scanned_cue_count` 必须等于 pack `cue_count`；同时完整覆盖所有 `required_decision_cue_numbers`。模型可对未提示 cue 主动提交 MASK/REVIEW。

`sensitive-finalize` 只接受 KEEP/MASK/REVIEW：KEEP/MASK 必须 high-confidence；MASK 只能提交 exact `{text,occurrence}` term，程序派生“首字符 + `*`”，不接受自由 replacement text。任何 REVIEW、漏审、stale pack/source、expected text/timing 不一致、term 不存在/重叠或输出覆盖旧文件均 fail closed。成功输出必须保持 cue count/number/start/end 与输入 SRT 完全一致，并写独立 audit；只有该输出是未来正常任务正式 Safe Final。该 gate 不授予 canonical/timing/segmentation authority，也不允许借 mask 修歌词。
...
显式模型 override 必须绑定 task fingerprint 与 `occurrence_id + track_id + canonical_line_index + expected_text`，且明确 `confidence=high`。expected text 不匹配、override 未命中/重复命中、policy task identity 不一致均必须 fail closed。新 production display 默认还必须同时提供 `--semantic-sensitive-pack` 与 `--semantic-sensitive-review`；两者先经完整 model-review lineage 校验，再仅物化 high-confidence exact-term mask。提供 semantic review 时 `display_policy.mask_profile` 必须为 `none`，禁止机械 profanity mask 与模型决策并行。旧任务无语义审查的重放只能显式使用 `--legacy-allow-no-semantic-sensitive-review`。
...
输出 audit 必须保留 canonical 原文及其 hash，以及原始 source start/end；同时把 `text/start_ms/end_ms` 绑定到最终 display 值，记录 display timing change reason、semantic-sensitive action/model/mask count，并重算 `text_sha256` / `cue_id`。QA、normalized config、artifact evidence 必须同时记录 semantic review model、pack/review SHA、full-scan、masked counts 与 unresolved=0。该阶段生成新的唯一 `final_render` artifact，以上一层 production final-render 为 upstream；release 时只提交新的 display final-render。`v4_validate_release.py` 新 production 默认要求这三层 semantic-sensitive lineage 一致；历史重放才允许显式 legacy 开关。
...
`v4_build_gap_ab_review.py --legacy-ui --job <job.json> --output-dir <new-directory> --expand-case <case-id> --context-seconds 30` 仅历史复现。原裁切和导出语义不变，不新增人工 authority。新任务复用 3.2 和统一 `v4_listen.py`；P1 review builder 的默认 `index.html` 也由同一 3.2 渲染，旧 response/ingest 不变。
